...
static bool is_at_end(Parser *parser) {
  return parser->current >= parser->count ||
         parser->tokens[parser->current].type == EOF_TOKEN;
}
...
static Expr *primary(Parser *parser, RuntimeError **error) {
    ...
    if (token->literal) {
      ...
      case LITERAL_STRING:
        if (token->literal->value.string) {
          ...
          memcpy(expr->as.literal.value.value.string,
                 token->literal->value.string, len + 1);
  ...
  if (match(parser, 1, LEFT_BRACKET)) {
    ...
    if (!check(parser, RIGHT_BRACKET)) {
      do {
        if (expr->as.list_literal.elements.count >=
            expr->as.list_literal.elements.capacity) {
          expr->as.list_literal.elements.capacity =
              expr->as.list_literal.elements.capacity == 0
                  ? 8
                  : expr->as.list_literal.elements.capacity * 2;
        ...
        expr->as.list_literal.elements
            .expressions[expr->as.list_literal.elements.count++] = element;
    ...
    do {
      if (expr->as.call.arguments.count >= expr->as.call.arguments.capacity) {
        expr->as.call.arguments.capacity =
            expr->as.call.arguments.capacity == 0
                ? 8
                : expr->as.call.arguments.capacity * 2;
      ...
      expr->as.call.arguments.expressions[expr->as.call.arguments.count++] =
          arg;
    } while (match(parser, 1, COMMA));
  }
...
static Expr *assignment(Parser *parser, RuntimeError **error) {
  ...
  if (match(parser, 1, ASSIGN)) {
    ...
    if (expr->type == EXPR_VARIABLE) {
      ...
      /* Prevent double-free of name.lexeme: transfer ownership to new assign
       * token */
      char *transferred_lexeme = name.lexeme;
      expr->as.variable.name.lexeme = NULL; /* so expr_free won't free it */
      expr_free(expr);

      Expr *assign = expr_new(EXPR_ASSIGN);
      assign->as.assign.name = name;
      assign->as.assign.name.lexeme =
          transferred_lexeme; /* reuse existing string */
...
StmtList parser_parse(Parser *parser, RuntimeError **error) {
  ...
  while (!is_at_end(parser) && !*error) {
    ...
    if (statements.count >= statements.capacity) {
      statements.capacity =
          statements.capacity == 0 ? 8 : statements.capacity * 2;
      statements.statements =
          realloc(statements.statements, statements.capacity * sizeof(Stmt *));
    }
    statements.statements[statements.count++] = stmt; /* store pointer */
  }
...
static Stmt *print_statement(Parser *parser, RuntimeError **error) {
  ...
  if (has_paren && check(parser, RIGHT_PAREN)) {
    // Allow empty print() just prints newline
    advance(parser); // consume RIGHT_PAREN
  } else {
    Expr *expr = expression(parser, error);
    if (*error) {
      stmt_free(stmt);
    ...
    while (match(parser, 1, COMMA)) {
      ...
      stmt->as.print.expressions =
          realloc(stmt->as.print.expressions,
                  (stmt->as.print.count + 1) * sizeof(Expr *));
      stmt->as.print.expressions[stmt->as.print.count++] = additional;
    }
    if (has_paren) {
      consume(parser, RIGHT_PAREN, "Expected ')' after arguments in print().",
              error);
      if (*error) {
        stmt_free(stmt);
        return NULL;
...
static Stmt *assert_statement(Parser *parser, RuntimeError **error) {
  /* 'assert' already consumed */
  Stmt *stmt = stmt_new(STMT_ASSERT);
  stmt->as.assert_stmt.keyword = *previous(parser);
  stmt->as.assert_stmt.keyword.lexeme = ms_strdup(previous(parser)->lexeme);
  stmt->as.assert_stmt.condition = NULL;
  stmt->as.assert_stmt.message = NULL;
  ...
  if (match(parser, 1, COMMA)) {
    Expr *message = expression(parser, error);
    if (*error) {
      stmt_free(stmt);
      return NULL;
    }
  ...
  while (!check(parser, RIGHT_BRACE) && !is_at_end(parser)) {
    ...
    if (stmt->as.block.statements.count >= stmt->as.block.statements.capacity) {
      stmt->as.block.statements.capacity =
          stmt->as.block.statements.capacity == 0
              ? 8
              : stmt->as.block.statements.capacity * 2;
    ...
    stmt->as.block.statements.statements[stmt->as.block.statements.count++] =
        declaration_stmt;
  }
...
static Stmt *statement(Parser *parser, RuntimeError **error) {
  ...
  if (match(parser, 1, ASSERT))
    return assert_statement(parser, error);
  if (match(parser, 1, RETURN))
    return return_statement(parser, error);
  if (match(parser, 1, WHILE))
    return while_statement(parser, error);
  if (match(parser, 1, LEFT_BRACE))
    return block_statement(parser, error);

  return expression_statement(parser, error);
}

static Stmt *function_declaration(Parser *parser, const char *kind,
                                  RuntimeError **error) {
  ...
  if (!check(parser, RIGHT_PAREN)) {
    do {
      ...
      Token *param =
          consume(parser, IDENTIFIER, "Expected parameter name.", error);
      if (*error) {
        free(params);
        return NULL;
      }
...
static Stmt *declaration(Parser *parser, RuntimeError **error) {
  if (match(parser, 1, FUNCTION))
    return function_declaration(parser, "function", error);
  if (match(parser, 1, VAR))
    return var_declaration(parser, error);
